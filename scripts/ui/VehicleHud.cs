...
/// <summary>
/// Reverse readout for the car the local player is driving. Builds its own label so it can be
/// dropped into any scene with nothing to wire up. Speed lives on the <see cref="Speedometer"/>
/// dial and nitro charges on the <see cref="NitroBar"/> bubbles, both bottom-left.
///
/// No revs or gear number, deliberately. The car has no gearbox — the revs and gear number the
/// engine note sweeps through are computed backwards from road speed (see
/// <see cref="FakeGearbox"/>), so showing them would just be a second speedometer dressed up as
/// a drivetrain.
/// </summary>
[GlobalClass]
